...
/*   Updated: 2023/12/24 15:02:20 by sangyepa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	add_dummy_to_dummy_list(t_list **dummy_list, \
										t_dummy_info *dummy_info)
{
	...
	ft_lstadd_back(dummy_list, ft_lstnew(dummy));
	if (dummy_info->type & DM_ADD_NULL)
	{
		...
		ft_lstadd_back(dummy_list, ft_lstnew(dummy_null));
	}
}
...
int	create_dummy_list(t_list **dummy_list, char *line)
{
	...
	while (*line)
	{
		...
		check_malloc_error(dummy_info);
		if ((dummy_info->type & DM_QUOTE) && !(dummy_info->type & DM_END))
		{
			free(dummy_info);
			return (FAIL);
		}
		add_dummy_to_dummy_list(dummy_list, dummy_info);
	}
	free(dummy_info);
	return (SUCCESS);
}
